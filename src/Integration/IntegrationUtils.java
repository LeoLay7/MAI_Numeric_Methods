...
public class IntegrationUtils {
    ...
    public static IntegrationResult midpointRule(IntegrationFunction function, double a, double b, double eps) {
        ...
        while (true) {
            ...
            double error = Math.abs(integral2 - integral1) / 3.0;
    ...
    public static double rungeErrorEstimate(IntegrationResult result, int order) {
        double I_h = result.getValueWithHalfStep();
        double I_h2 = result.getValue();
        
        // оценка погрешности уточненного значения: |I_h/2 - I_h|/(2^p - 1)²
        double baseError = Math.abs(I_h2 - I_h) / (Math.pow(2, order) - 1);
        return baseError / (Math.pow(2, order) - 1);
    }
}